...
::: {.callout-important}
Não entendi direito, pedir ajuda para o Julio
:::
...
::: {.callout-important}
TO DO: entender. O que aconteceu nesse passo?
:::


$$
= \lim_{x \to p} \frac{f(x) \cdot g(x) - f(p) \cdot g(p) - f(p) \cdot g(x) + f(p) \cdot g(x)}{x - p}
$$
...
$$
\lim_{x \to p} h'(x) = g(p) \cdot f'(p) + f(p) \cdot g'(p)
$$
